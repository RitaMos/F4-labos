...
\begin{longtable}{@{}L{3.1cm}L{5.2cm}L{4.6cm}@{}}
    ...
    Fuente regulable de alta tensión & Genera la diferencia de potencial entre las placas. & 0 a 300 V aprox.; modelo: [COMPLETAR] \\
    Tester o multímetro & Controla la tensión aplicada a la celda. & Resolución: [COMPLETAR] V \\
    Llave selectora & Conecta, desconecta o invierte la polaridad del campo. & Posiciones utilizadas: campo y neutro \\
    Cronómetro & Mide el tiempo de caída sobre seis divisiones. & Resolución: [COMPLETAR] s \\
...
\begin{tabular}{@{}C{1.8cm}C{2.9cm}C{2.2cm}C{3.3cm}C{2.2cm}@{}}
    \toprule
    \textbf{Medición} & \textbf{$V$ equilibrio (V)} & \textbf{$\Delta V$ (V)} & \textbf{$t$ para 6 div. (s)} & \textbf{$\Delta t$ (s)} \\
    \midrule
...
\centering
\small
\caption{Constantes de cálculo. Reemplazar los valores típicos si se usaron valores medidos.}
...
\begin{tabular}{@{}C{1.2cm}C{2.5cm}C{2.3cm}C{2.6cm}C{2.6cm}C{1.3cm}@{}}
    \toprule
    \textbf{Med.} & \textbf{$v_L$ (m/s)} & \textbf{$a$ (m)} & \textbf{$q$ (C)} & \textbf{$\Delta q$ (C)} & \textbf{$n_i$} \\
    \midrule
    ...
    7 & [\quad] & [\quad] & [\quad] & [\quad] & [\quad] \\
    8 & [\quad] & [\quad] & [\quad] & [\quad] & [\quad] \\
    9 & [\quad] & [\quad] & [\quad] & [\quad] & [\quad] \\
    10 & [\quad] & [\quad] & [\quad] & [\quad] & [\quad] \\
    11 & [\quad] & [\quad] & [\quad] & [\quad] & [\quad] \\
    12 & [\quad] & [\quad] & [\quad] & [\quad] & [\quad] \\
    \bottomrule
\end{tabular}
\end{table}
...
Durante la práctica se dificultó la visualización de las gotas debido a la iluminación, el contraste
su reducido tamaño y la alta concentración de estas. Esto ocurrio a distintos niveles de gravedad dependiendo del 
...
\textbf{Cuantización observada:} \pendiente{INDICAR SI LAS CARGAS $q_i$ SE AGRUPAN ALREDEDOR DE MÚLTIPLOS ENTEROS Y MENCIONAR EL INTERVALO DE $n_i$ OBTENIDO}

\textbf{Dispersión y compatibilidad:} \pendiente{DESCRIBIR LA DISPERSIÓN DE $q$ Y SI EL RESULTADO $e_{\mathrm{exp}}$ ES COMPATIBLE CON $e_{\mathrm{ref}}$ DENTRO DE $\Delta e$}

\textbf{Datos atípicos:} \pendiente{IDENTIFICAR EVENTUALES VALORES ALEJADOS Y JUSTIFICAR CUALQUIER DESCARTE CON LA MEDICIÓN Y SU INCERTIDUMBRE}

La guía advierte que no debe eliminarse una medición únicamente porque no coincide con el valor esperado. Un descarte requiere una causa experimental identificable. También conviene verificar que el conjunto incluya al menos una carga asociada a un múltiplo impar; si todas fueran pares, el ajuste podría seleccionar erróneamente un submúltiplo.
...
Mediante la combinación de la caída a velocidad terminal y el equilibrio electrostático se obtuvo la carga de cada gota sin medir directamente su masa ni su radio. El valor de la carga elemental deducido del conjunto fue \pendiente{$e_{\mathrm{exp}}\pm\Delta e$}. La diferencia relativa respecto del valor de referencia fue \pendiente{$\varepsilon_r$}. \pendiente{INDICAR SI EL RESULTADO FUE COMPATIBLE Y CUÁL FUE LA LIMITACIÓN EXPERIMENTAL DOMINANTE}

\section{Referencias}

\begin{enumerate}[label={[\arabic*]},leftmargin=1.05cm]
    \item Laboratorio de Física IV, Instituto Tecnológico de Buenos Aires. \textit{Experimento de Millikan}. Guía de trabajo práctico, 2017.
    \item Laboratorio de Física IV, Instituto Tecnológico de Buenos Aires. \textit{Guía para la presentación de un informe}. Documento de cátedra.
\end{enumerate}
